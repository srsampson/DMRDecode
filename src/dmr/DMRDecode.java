...
public class DMRDecode {
    ...
    private void frameCalcs(int lmin, int lmax) {
        ...
        if (debug == true) {
            String l = getTimeStamp() + " Setting new params : center=" + Integer.toString(centre) + " max=" + Integer.toString(max) + " min=" + Integer.toString(min) + " umid=" + Integer.toString(umid) + " lmid=" + Integer.toString(lmid);
    ...
    public int getFrameSync() {
        ...
        while (true) {
            ...
            if (enableDisplayBar == true) {
                window.displaySymbol(symbol);
            }
            // Set the dibit state
            ...
            if (t >= 144) {
                ...
                if ((frameSync == false) || ((frameSync == true) && (symbolcnt % 144 == 0))) {
                    ...
                    lmax = -1;
                    for (a = 0; a < 24; a++) {
                        if (syncHighLowlBuf[a] < lmin) {
                            lmin = syncHighLowlBuf[a];
                        }
                        if (syncHighLowlBuf[a] > lmax) {
                            lmax = syncHighLowlBuf[a];
                        }
                    }
                }
    ...
    void addToDitbitBuf(int dibit) {
        ...
        if (dibitCircularBufferCounter == 144) {
            dibitCircularBufferCounter = 0;
        }
    }
    ...
    void addToSymbolBuffer(int symbol) {
        ...
        symbolBufferCounter++;
        if (symbolBufferCounter == 144) {
            symbolBufferCounter = 0;
        }
    }
    ...
    private int syncCompare(boolean sync) {
        ...
        for (i = 0; i < 24; i++) {
            // BS
            if (dibitCircularBuffer[circPos] == DMR_VOICE_SYNC_BS[i]) {
                voiceSyncBS++;
            }
            if (dibitCircularBuffer[circPos] == DMR_DATA_SYNC_BS[i]) {
                dataSyncBS++;
            }
            // MS
            if (dibitCircularBuffer[circPos] == DMR_VOICE_SYNC_MS[i]) {
                voiceSyncMS++;
            }
            if (dibitCircularBuffer[circPos] == DMR_DATA_SYNC_MS[i]) {
                dataSyncMS++;
            }
            if (dibitCircularBuffer[circPos] == DMR_RC_SYNC[i]) {
                rcSync++;
            }
            // Direct Slot 1
            if (dibitCircularBuffer[circPos] == DMR_VOICE_SYNC_DIRECT1[i]) {
                directVoice1++;
            }
            if (dibitCircularBuffer[circPos] == DMR_DATA_SYNC_DIRECT1[i]) {
                directData1++;
            }
            // Direct Slot 2
            if (dibitCircularBuffer[circPos] == DMR_VOICE_SYNC_DIRECT2[i]) {
                directVoice2++;
            }
            if (dibitCircularBuffer[circPos] == DMR_DATA_SYNC_DIRECT2[i]) {
                directData2++;
            }
            // Rest Data
            if (dibitCircularBuffer[circPos] == DMR_DATA_REST_SYNC_BS[i]) {
                restData++;
            }
        ...
        if ((DMR_VOICE_SYNC_BS.length - voiceSyncBS) <= diff) {
            return 1;
        } else if ((DMR_DATA_SYNC_BS.length - dataSyncBS) <= diff) {
            return 2;
        } else if ((DMR_VOICE_SYNC_MS.length - voiceSyncMS) <= diff) {
            return 3;
        } else if ((DMR_DATA_SYNC_MS.length - dataSyncMS) <= diff) {
            return 4;
        } else if ((DMR_RC_SYNC.length - rcSync) <= diff) {
            return 5;
        } else if ((DMR_VOICE_SYNC_DIRECT1.length - directVoice1) <= diff) {
            return 6;
        } else if ((DMR_DATA_SYNC_DIRECT1.length - directData1) <= diff) {
            return 7;
        } else if ((DMR_VOICE_SYNC_DIRECT2.length - directVoice2) <= diff) {
            return 8;
        } else if ((DMR_DATA_SYNC_DIRECT2.length - directData2) <= diff) {
            return 9;
        } else if ((DMR_DATA_REST_SYNC_BS.length - restData) <= diff) {
            return 10;
        } else {
            return 0;
        }
    ...
    void processFrame() {
        ...
        // Deal with the frame
        if ((synctype == 12) || (synctype == 22) || (synctype == 30) || (synctype == 32)) {
            processDMRvoice();
        } else if ((synctype == 10) || (synctype == 20) || (synctype == 31) || (synctype == 33) || (synctype == 40)) {
            processDMRdata();
        } else if (synctype == 13) {
            processEmbedded();
        }
    ...
    void processDMRvoice() {
        ...
        if ((DMRvoice.isError() == false) && (displayOnlyGoodFrames == true)) {
            DMRvoice.setShouldDisplay(false);
        }
        // Display the info
        if ((DMRvoice.getShouldDisplay() == true) && (displayVoiceFrames == true)) {
    ...
    void processDMRdata() {
        ...
        // If this frame contains errors and the user wants to display only good ones
        // then stop them being shown
        if ((DMRdata.isError() == false) && (displayOnlyGoodFrames == true)) {
            DMRdata.setShouldDisplay(false);
        }
        // Display the info
        if (DMRdata.getShouldDisplay() == true) {
            displayLines(line, lcol, font);
        }
    }
    ...
    void processEmbedded() {
        ...
        // If this frame contains errors and the user wants to display only good ones
        // then stop them being shown
        if ((DMRembedded.isError() == false) && (displayOnlyGoodFrames == true)) {
            DMRembedded.setShouldDisplay(false);
        }
        // Display the info
        if (DMRembedded.getShouldDisplay() == true) {
            displayLines(line, lcol, font);
        }
    }
    ...
    private void addToSamplesAheadBuffer(int sam) {
        ...
        samplesAheadCounter++;
        if (samplesAheadCounter == SAMPLESAHEADSIZE) {
            samplesAheadCounter = 0;
        }
    }
    ...
    private int getSample(boolean jitmode) {
        ...
        if ((captureMode == true) && (jitmode == false)) {
            audioDump(sample);
        }
    ...
    public int getCurrentIncomingDataType() {
        if (currentChannel == 1) {
            return incomingDataType[0];
        } else {
            return incomingDataType[1];
        }
    }
    ...
    public void setCurrentDataBlocksToFollow(int blocks) {
        if (currentChannel == 1) {
            dataBlocksToFollow[0] = blocks;
        } else {
            dataBlocksToFollow[1] = blocks;
        }
    }
    ...
    public int getCurrentDataBlocksToFollow() {
        if (currentChannel == 1) {
            return dataBlocksToFollow[0];
        } else {
            return dataBlocksToFollow[1];
        }
    }
    ...
    public int getCurrentDataBlocksReceived() {
        if (currentChannel == 1) {
            return dataBlocksReceived[0];
        } else {
            return dataBlocksReceived[1];
        }
    }
