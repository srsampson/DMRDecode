...
public class CSBK {
    ...
    private void csbko25fid0(DMRDecode theApp, boolean bits[]) {
        ...
        // Infill
        if (bits[22] == true) {
            sb1.append("Infill Radio Site : ");
        } else {
            sb1.append("Not an Infill Radio Site : ");
        }
        // Active_Connection
        if (bits[23] == true) {
            sb1.append("TS has Network Connection : ");
        } else {
            sb1.append("TS doesn't have a Network Connection : ");
        }
        // Mask
        int mask = 0;
        if (bits[24] == true) {
    ...
    private void csbko28fid0(DMRDecode theApp, boolean bits[]) {
        ...
        if (ambient_listening_service == true) {
            display[0] = display[0] + " : ALS Requested : ";
        }
        if (appended_blocks > 0) {
            display[0] = display[0] + Integer.toString(appended_blocks) + " Blocks Appended : ";
        }
        display[0] = display[0] + "SOM=" + Integer.toString(service_options_mirror);
        if (service_kind_flag == true) {
            display[0] = display[0] + " : SKF=1";
        } else {
    ...
    private void csbko40fid0(DMRDecode theApp, boolean bits[]) {
        ...
        if (at == 0) {
            ...
            if (utc_offset_fraction == 1) {
                sb1.append(" (Add 15 mins)");
            } else if (utc_offset_fraction == 1) {
                sb1.append(" (Add 30 mins)");
            } else if (utc_offset_fraction == 2) {
                sb1.append(" (Add 45 mins)");
            }
            display[1] = sb1.toString();
        } else if (at == 4) {
            ...
            aType = "Reserved (" + Integer.toString(at) + ")";
        }
        // System Identity Code
        int sysID = utils.retSixteen(bits, 40);
    ...
    private void csbko30fid0(DMRDecode theApp, boolean bits[]) {
        ...
        // Display this
        if (skf == true) {
            sb1.append("Service_Kind_Flag=1 : ");
        } else {
    ...
    private void csbko42fid0(DMRDecode theApp, boolean bits[]) {
        ...
        if (maint_kind == 0) {
            mks = "Disconnect. End of payload channel use";
        } else {
            mks = "Reserved";
        }
    ...
    private void big_m_csbko59(DMRDecode theApp, boolean bits[]) {
        ...
        if (bits[19] == true) {
            restCh = 16;
        } else if (bits[20] == true) {
            restCh = restCh + 8;
        } else if (bits[21] == true) {
            restCh = restCh + 4;
        } else if (bits[22] == true) {
            restCh = restCh + 2;
        ...
        if (bits[24] == false) {
            ...
            sb1.append(" : Asynchronous Beacons");
        }
    ...
    private void csbko48fid0(DMRDecode theApp, boolean bits[]) {
        ...
        if (bits[29] == true) {
            sb1.append(": OVCM Call ");
        }
        if (bits[30] == true) {
            sb1.append(": Emergency Call ");
        }
        if (bits[31] == false) {
            sb1.append(": Aligned Timing");
        } else {
            sb1.append(": Offset Timing");
        }
        display[1] = sb1.toString();
        // Target address
        int target = utils.retAddress(bits, 32);
        ...
        if (theApp.currentChannel == 1) {
            theApp.setCh1Label(lab.toString(), theApp.labelBusyColour);
        } else {
            theApp.setCh2Label(lab.toString(), theApp.labelBusyColour);
        }
        // Quick log
        if (theApp.isQuickLog() == true) {
            theApp.quickLogData("Private Voice Channel Grant", target, source, lchannel, display[1]);
        }
    }
    ...
    private void csbko49fid0(DMRDecode theApp, boolean bits[]) {
        ...
        // Quick log
        if (theApp.isQuickLog() == true) {
            theApp.quickLogData("Talkgroup Voice Channel Grant", target, source, lchannel, display[1]);
        }
    }
    ...
    private void csbko50fid0(DMRDecode theApp, boolean bits[]) {
        ...
        // Quick log
        if (theApp.isQuickLog() == true) {
            theApp.quickLogData("Broadcast Talkgroup Voice Channel Grant", target, source, lchannel, display[1]);
        }
    }
    ...
    private void csbko51fid0(DMRDecode theApp, boolean bits[]) {
        ...
        if (bits[29] == true) {
            sb1.append(": Payload Channel uses 1:1 mode ");
        } else {
            sb1.append(": Payload Channel uses 2:1 mode ");
        }
        if (bits[30] == true) {
            sb1.append(": Emergency Call ");
        }
        ...
        // Quick log
        if (theApp.isQuickLog() == true) {
            theApp.quickLogData("Private Data Channel Grant", target, source, lchannel, display[1]);
        }
    }
    ...
    private void csbko52fid0(DMRDecode theApp, boolean bits[]) {
        ...
        // Quick log
        if (theApp.isQuickLog() == true) {
            theApp.quickLogData("Talkgroup Data Channel Grant", target, source, lchannel, display[1]);
        }
    }
