...
public class DMRData {
    ...
    private void responsePacket(boolean bits[]) {
        ...
        if ((dclass == 0) && (type == 1)) {
            ...
            sb.append("NACK (Illegal Format)");
        } else if ((dclass == 1) && (type == 1)) {
            sb.append("NACK (CRC Failed)");
        } else if ((dclass == 1) && (type == 2)) {
            sb.append("NACK (Memory Full)");
        } else if ((dclass == 1) && (type == 4)) {
            sb.append("NACK (Undeliverable)");
    ...
    private void handleConfirmedData(boolean bits[]) {
        ...
        if (bits.length == 96) {
            display[0] = "R_1_2_DATA (data block serial number=" + Integer.toString(dbsn) + ")";
        } else if (bits.length == 144) {
            display[0] = "R_3_4_DATA (data block serial number=" + Integer.toString(dbsn) + ")";
        }
