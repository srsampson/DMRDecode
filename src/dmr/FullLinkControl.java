...
public class FullLinkControl {
    ...
    public String[] decode(DMRDecode theApp, boolean bits[]) {
        ...
        switch (flco) {
            case 0:
                group_v_ch_usr(theApp, bits);
                break;
            case 3:
                uu_v_ch_usr(theApp, bits);
                break;
            case 4:
                big_m_flco4(theApp, bits);
                break;
            case 48:
                td_lc(theApp, bits);
                break;
            default:
                unknown_flc(flco, fid, bits);
                break;
        }
        return display;
    }
    ...
    void group_v_ch_usr(DMRDecode theApp, boolean bits[]) {
        ...
        sb.append("Group Address : ").append(Integer.toString(group));
        sb.append(" Source Address : ").append(Integer.toString(source));
        ...
        if (theApp.isQuickLog() == true) {
            theApp.quickLogData("Group Voice Call to Group", group, source, theApp.currentChannel, display[1]);
        }
    }
    ...
    void uu_v_ch_usr(DMRDecode theApp, boolean bits[]) {
        ...
        // Source address
        int source = utils.retAddress(bits, 48);
        sb.append("Target Address : ").append(Integer.toString(target));
        sb.append(" Source Address : ").append(Integer.toString(source));
        display[2] = sb.toString();
        // Log these users
        // Target
        ...
        // Quick log
        if (theApp.isQuickLog() == true) {
            theApp.quickLogData("Unit to Unit Voice Call", target, source, theApp.currentChannel, display[1]);
        }
    }
    ...
    void td_lc(DMRDecode theApp, boolean bits[]) {
        ...
        sb.append("Destination Logical Link ID : ").append(Integer.toString(dllid));
        sb.append(" Source Logical Link ID : ").append(Integer.toString(sllid));
        ...
        // Quick log
        if (theApp.isQuickLog() == true) {
            theApp.quickLogData("Terminator Data Link Control PDU", dllid, sllid, theApp.currentChannel, "");
        }
    }
    ...
    void big_m_flco4(DMRDecode theApp, boolean bits[]) {
        ...
        sb1.append("Group Address ").append(Integer.toString(group)).append(" Source Address ").append(Integer.toString(source)).append(" LCN ").append(Integer.toString(lcn));
        ...
        // Quick log
        if (theApp.isQuickLog() == true) {
            theApp.quickLogData("Capacity Plus Full Link Control LC", group, source, theApp.currentChannel, "");
        }
    }
    ...
    private void unknown_flc(int flco, int fid, boolean bits[]) {
        ...
        sb.append("Unknown Full Link Control LC : FLCO=").append(Integer.toString(flco)).append(" + FID=").append(Integer.toString(fid)).append(" ");
        // Display the binary
        for (a = 16; a < 72; a++) {
            if (bits[a] == true) {
                sb.append("1");
